...
const FIRST  = ["James","Mary","Robert","Patricia","John","Jennifer","Michael","Linda","David","Barbara","William","Susan","Richard","Jessica","Joseph","Sarah","Thomas","Karen","Charles","Lisa","Christopher","Nancy","Daniel","Betty","Matthew","Sandra","Anthony","Margaret","Mark","Ashley","Donald","Kimberly","Steven","Emily","Paul","Donna","Andrew","Michelle","Kenneth","Carol","Joshua","Amanda","Kevin","Melissa","Brian","Deborah","George","Stephanie","Timothy","Rebecca"];

const LAST   = ["Smith","Johnson","Williams","Brown","Jones","Garcia","Miller","Davis","Martinez","Hernandez","Lopez","Gonzalez","Wilson","Anderson","Thomas","Taylor","Moore","Jackson","Martin","Lee","Perez","Thompson","White","Harris","Sanchez","Clark","Ramirez","Lewis","Robinson","Walker","Young","Allen","King","Wright","Scott","Torres","Nguyen","Hill","Flores","Green","Adams","Nelson","Baker","Hall","Rivera","Campbell","Mitchell","Carter","Roberts","Turner"];
  ...
  for (let i = 1; i <= 48; i++) {
    ...
    const vehicles = Array.from({ length: numVehicles }, (_, vi) => {
      ...
      return {
        ...
        status,
        nextBillingDate: status === "Cancelled" ? null : new Date(Date.now() + Math.floor(vm() * 30) * 24 * 3600 * 1000).toISOString().split("T")[0],
    ...
    const purchases = Array.from({ length: numPurchases }, (_, pi) => {
      ...
      return {
        ...
        date:        genDate(pr, 1),
        type,
        description: type === "Subscription" ? `${wash} Monthly Membership` : `${wash} - Single Wash`,
        amount:      type === "Subscription" ? PLAN_PRICES[wash] : (PLAN_PRICES[wash] * 0.7).toFixed(2),
    ...
    users.push({
      ...
      city:          pick(["Los Angeles","Houston","Miami","New York","Seattle","Denver","Chicago","Atlanta","Phoenix","Charlotte"], r),
      state:         pick(STATES, r),
      zip:           `${Math.floor(10000 + r() * 90000)}`,
      accountStatus: pick(["Active","Active","Active","Suspended","Inactive"], r),
      vehicles,
      purchases,
    });
  }
