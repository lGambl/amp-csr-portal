...
const MAKES  = ["Toyota","Honda","Ford","Chevrolet","BMW","Tesla","Nissan","Hyundai","Jeep","Subaru"];
const MODELS = {
  Toyota: ["Camry","Corolla","RAV4"],       Honda:     ["Civic","Accord","CR-V"],
  Ford:   ["F-150","Mustang","Explorer"],   Chevrolet: ["Silverado","Equinox","Malibu"],
  BMW:    ["3 Series","5 Series","X5"],     Tesla:     ["Model 3","Model Y","Model S"],
  Nissan: ["Altima","Rogue","Sentra"],       Hyundai:   ["Elantra","Tucson","Sonata"],
  Jeep:   ["Wrangler","Cherokee","Grand Cherokee"], Subaru: ["Outback","Forester","Impreza"],
};
const COLORS  = ["White","Black","Silver","Gray","Red","Blue","Green","Brown","Beige","Orange"];
...
const EMPTY_FORM  = { year: "2022", make: "Toyota", model: "Camry", color: "White", plate: "", plan: "Basic", status: "Active" };
    ...
    <Box sx={sx.vehicleFormGrid}>
      ...
      <Box>
        ...
        <Select value={form.plan} onChange={set("plan")} size="small" fullWidth sx={sx.selectField} MenuProps={sx.menuProps}>
          {WASH_PLANS.map(p => <MenuItem key={p} value={p} sx={sx.menuItem}>{p} — ${PLAN_PRICES[p]}/mo</MenuItem>)}
        </Select>
      </Box>
      <Box>
  ...
  return (
    <Box sx={sx.vehicleCard}>
      <Box sx={sx.vehicleCardHeader}>
        <Box>
          <Typography sx={sx.vehicleTitle}>{vehicle.year} {vehicle.make} {vehicle.model}</Typography>
          <Typography sx={sx.vehicleMeta}>{vehicle.color} · {vehicle.plate}</Typography>
        </Box>
      ...
      <Box sx={sx.vehicleDetailsGrid}>
        ...
        <Box>
          ...
          <Typography sx={sx.fieldValueAccent}>${vehicle.planPrice}/mo</Typography>
        </Box>
        <Box>
          <Typography sx={sx.fieldLabel}>Start Date</Typography>
          <Typography sx={sx.fieldValue}>{vehicle.startDate}</Typography>
        </Box>
        <Box>
          <Typography sx={sx.fieldLabel}>Next Billing</Typography>
          <Typography sx={sx.fieldValue}>{vehicle.nextBillingDate ?? "—"}</Typography>
        </Box>
        <Box>
      ...
      <Box sx={sx.vehicleActions}>
        <Button size="small" onClick={onEdit} sx={sx.editBtn}>Edit</Button>
        <Button size="small" onClick={onTransfer} disabled={!canTransfer} sx={sx.outlineBtn}>Transfer Plan</Button>
        <Button size="small" onClick={onRemove} sx={sx.dangerBtn}>Remove</Button>
      </Box>
    </Box>
  );
}
...
export default function Vehicles({ user, onUpdateUser }) {
  ...
  const saveEdit = () => {
    updateVehicles(user.vehicles.map(v =>
      v.id === editForm.id ? { ...editForm, planPrice: PLAN_PRICES[editForm.plan] } : v
    ));
    setEditForm(null);
  };
  ...
  const addVehicle = () => {
    const newV = {
      ...
      nextBillingDate: newForm.status === "Cancelled" ? null
  ...
  const doTransfer = () => {
    const nextBilling = new Date(Date.now() + 30 * 24 * 3600 * 1000).toISOString().split("T")[0];
    updateVehicles(user.vehicles.map(v => {
      if (v.id === transferSource.id)  return { ...v, status: "Cancelled", nextBillingDate: null };
      if (v.id === transferTargetId)   return { ...v, plan: transferSource.plan, planPrice: transferSource.planPrice, status: "Active", nextBillingDate: nextBilling };
      return v;
    }));
  ...
  return (
    <Box>
      <Box sx={sx.sectionHeader}>
        <Typography sx={sx.sectionTitle}>Vehicle Subscriptions</Typography>
        <Button onClick={() => setAddOpen(true)} sx={sx.saveBtn}>+ Add Vehicle</Button>
      </Box>
      ...
      <Box sx={sx.vehicleGrid}>
        {user.vehicles.map(v => (
          <VehicleCard
            ...
            onTransfer={() => { setTransferSource(v); setTransferTargetId(""); }}
          />
        ))}
      </Box>

      {/* Edit Vehicle */}
      <Dialog open={!!editForm} onClose={() => setEditForm(null)} maxWidth="xs" fullWidth PaperProps={{ sx: sx.dialogPaper }}>
        <DialogTitle sx={sx.dialogTitle}>Edit Vehicle</DialogTitle>
        <DialogContent>
          {editForm && <VehicleForm form={editForm} setForm={setEditForm} />}
        </DialogContent>
        <DialogActions sx={sx.dialogActions}>
          <Button onClick={() => setEditForm(null)} sx={sx.cancelBtn}>Cancel</Button>
          <Button onClick={saveEdit} sx={sx.saveBtn}>Save</Button>
        </DialogActions>
      </Dialog>

      {/* Add Vehicle */}
      <Dialog open={addOpen} onClose={() => setAddOpen(false)} maxWidth="xs" fullWidth PaperProps={{ sx: sx.dialogPaper }}>
        <DialogTitle sx={sx.dialogTitle}>Add Vehicle</DialogTitle>
        <DialogContent>
          <VehicleForm form={newForm} setForm={setNewForm} />
        </DialogContent>
        <DialogActions sx={sx.dialogActions}>
          <Button onClick={() => setAddOpen(false)} sx={sx.cancelBtn}>Cancel</Button>
          <Button onClick={addVehicle} disabled={!newForm.plate} sx={sx.saveBtn}>Add Vehicle</Button>
        </DialogActions>
      </Dialog>

      {/* Remove Confirmation */}
      <Dialog open={!!removeTarget} onClose={() => setRemoveTarget(null)} maxWidth="xs" fullWidth PaperProps={{ sx: sx.dialogPaper }}>
        <DialogTitle sx={sx.dialogTitle}>Remove Vehicle</DialogTitle>
        <DialogContent>
          <Typography sx={sx.dialogWarnText}>
            Remove the {removeTarget?.year} {removeTarget?.make} {removeTarget?.model} ({removeTarget?.plate}) and cancel its {removeTarget?.plan} subscription? This cannot be undone.
          </Typography>
        </DialogContent>
        <DialogActions sx={sx.dialogActions}>
          <Button onClick={() => setRemoveTarget(null)} sx={sx.cancelBtn}>Cancel</Button>
          <Button onClick={confirmRemove} sx={sx.dangerBtn}>Remove Vehicle</Button>
        </DialogActions>
      </Dialog>

      {/* Transfer Plan */}
      <Dialog open={!!transferSource} onClose={() => setTransferSource(null)} maxWidth="xs" fullWidth PaperProps={{ sx: sx.dialogPaper }}>
        <DialogTitle sx={sx.dialogTitle}>Transfer Plan</DialogTitle>
        <DialogContent>
          <Typography sx={sx.dialogBodyText}>
            Move the <strong style={sx.strongText}>{transferSource?.plan}</strong> plan
            from <strong style={sx.strongText}>{transferSource?.plate}</strong> to:
          </Typography>
          <Select
            value={transferTargetId}
            onChange={e => setTransferTargetId(e.target.value)}
            displayEmpty size="small" fullWidth
            sx={sx.selectField} MenuProps={sx.menuProps}
          >
        ...
        <DialogActions sx={sx.dialogActions}>
          <Button onClick={() => setTransferSource(null)} sx={sx.cancelBtn}>Cancel</Button>
