    ...
    return [
        ...
        },
        {
            ...
            value: users.reduce(
                (s, u) =>
                    s +
                    u.subscriptions.filter((sub) => sub.status === "Active")
                        .length,
                0,
            ),
            colorKey: "accent",
        },
        {
            ...
            value: users.reduce(
                (s, u) =>
                    s +
                    u.subscriptions.filter((sub) => sub.status === "Overdue")
                        .length,
                0,
            ),
            colorKey: "red",
    ...
    return (
        <div className={styles.wrapper}>
            <div className={styles.statGrid}>
                {[...Array(4)].map((_, i) => (
                    ...
                    >
                        <div
                            className="skeleton"
                            style={{ height: 30, width: 56, marginBottom: 10 }}
                        />
                        <div
                            className="skeleton"
            ...
            <div className={styles.toolbar}>
                ...
                <div
                    className="skeleton"
                    style={{ height: 36, width: 140, borderRadius: 4 }}
                />
                <div
                    className="skeleton"
                    style={{ height: 36, width: 140, borderRadius: 4 }}
...
export default function UserList({ users, onSelectUser }) {
    ...
    const filtered = useMemo(() => {
        ...
        return users.filter((u) => {
            const vehicleTerms = u.vehicles.flatMap((v) => [
                v.id,
                v.plate,
                v.make,
                v.model,
            ...
            <div className={styles.statGrid}>
                {stats.map((s) => (
                    <div
                        key={s.label}
                        className={styles.statCard}
                        data-color={s.colorKey}
                    >
                        <p className={styles.statNum} data-color={s.colorKey}>
                            {s.value}
                        </p>
